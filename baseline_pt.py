...
def build_estimator(self):
    if self.stage in ["evaluate", "offline_train"]:
        ...
        stage = "online_train"
    model_saved_stage_dir = os.path.join(args.model_saved_dir, stage, self.action)
    if not os.path.exists(model_saved_stage_dir):
        # 如果模型目录不存在，则创建该目录
        os.makedirs(model_saved_stage_dir)
    elif self.stage in ["online_train", "offline_train"]:
        # 训练时如果模型目录已存在，则清空目录
        del_file(model_saved_stage_dir)
    optimizer = AdamW(self.model.parameters(),lr=args.learning_rate, beta1=0.9, beta2=0.999,
                                       epsilon=1)
